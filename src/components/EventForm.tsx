...
const BLANK: MarathonEventInput = {
  name: '', eventType: 'half', plannedDate: '',
  city: '', state: 'CA', country: 'United States',
};
...
export default function EventForm({ editTarget, onClose }: Props) {
  const createEvent = useCreateEvent();
  const updateEvent = useUpdateEvent();
  const [form, setForm] = useState<MarathonEventInput>(BLANK);
  const [raceDate, setRaceDate] = useState<Date | null>(null);
  const [errors, setErrors] = useState<Partial<Record<keyof MarathonEventInput, string>>>({});
  ...
  function set<K extends keyof MarathonEventInput>(key: K, val: MarathonEventInput[K]) {
    setForm(f => ({ ...f, [key]: val }));
  ...
  const FIELD = 'w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500';
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            />
            {errors.plannedDate && <p className="text-red-500 text-xs mt-1">{errors.plannedDate}</p>}
          </div>

          {/* Goal Finish Time — always shown */}
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Goal Finish Time</label>
            <MarathonTimeInput
          ...
          {isCompleted && (
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Finished Time</label>
              <MarathonTimeInput
          ...
          <div>
            ...
            <input
              ...
              className={`${FIELD} ${errors.city ? 'border-red-400' : 'border-slate-200'}`}
            />
