...
const CARD = 'bg-white rounded-[4px] shadow-[0_8px_24px_rgba(13,13,18,0.04)] mb-3 relative p-5';
...
export default function SummaryPanel({ events, finished, planned }: Props) {
  ...
  return (
    <div>
      {/* Total */}
      <div className={CARD}>
        <div className="text-4xl font-bold text-slate-900 tracking-tight">{events.length}</div>
        <div className="text-xs text-slate-400 uppercase tracking-widest mt-1 font-medium">Total Races</div>
        <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
          <span>{statesCovered} state{statesCovered !== 1 ? 's' : ''}</span>
          <span>{new Set(events.map(e => e.country)).size} countr{new Set(events.map(e => e.country)).size !== 1 ? 'ies' : 'y'}</span>
        </div>
      </div>
      ...
      <div className={CARD}>
        <div className="flex items-baseline justify-between mb-3">
          <div>
            <div className="text-3xl font-bold text-slate-900">{finished.length}</div>
            <div className="text-xs text-slate-400 uppercase tracking-widest mt-0.5 font-medium">Finished</div>
          </div>
          <span className="text-2xl">🏅</span>
        </div>

        {finished.length === 0 ? (
          <p className="text-xs text-slate-400 italic">No finished races yet.</p>
        ) : (
      ...
      <div className={CARD}>
        <div className="flex items-baseline justify-between mb-3">
          <div>
            <div className="text-3xl font-bold text-slate-900">{planned.length}</div>
            <div className="text-xs text-slate-400 uppercase tracking-widest mt-0.5 font-medium">Planned</div>
          </div>
          <span className="text-2xl">📋</span>
        </div>
          ...
          <div className="pt-3 border-t border-slate-100">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              {(() => {
                ...
                return (
                  <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-semibold ${t.bgClass} ${t.textColorClass}`}>
                    {t.text}
                  </span>
                );
              })()}
